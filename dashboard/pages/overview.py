...
from dashboard.loaders import (
    load_analysis_results, load_db_recommendations, load_exec_logs,
    load_report, load_stock_names, load_stock_prices,
    parse_market_summary, parse_recs_from_report,
)
...
def page_today(selected_date: date) -> None:
    ...
    today_log = logs[logs["date"] == selected_date] if not logs.empty else __import__("pandas").DataFrame()
    if today_log.empty:
        try:
    ...
    watch_df = results_df[(results_df["total"] >= 55) & (results_df["total"] < 65)] if not results_df.empty else __import__("pandas").DataFrame()

...
def page_search() -> None:
    ...
    try:
        history = load_stock_history(sid)
        recs = history["recs"]
        ...
        if recs:
            ...
            for r in recs:
                ...
                color = {"A+": "#00c851", "A": "#33b5e5", "B": "#ffbb33"}.get(
                    level, "#aaa"
                )
                st.markdown(
                    f"""
...
def page_history() -> None:
    ...
    if not recent.empty:
        ...
        with col1:
            freq = (
                recent.groupby("stock_id")
                .size()
                .reset_index(name="次數")
                .sort_values("次數", ascending=False)
                .head(10)
            )
            freq["label"] = freq["stock_id"].apply(
                lambda s: f"{stock_names.get(s, s)}\n{s}"
            )
            fig = px.bar(
                freq,
                x="label",

...
def fetch_mops_announcements(trade_date: Optional[date] = None) -> List[dict]:
    ...
    url = (
        f"https://mops.twse.com.tw/mops/web/ajax_t38s000"
        f"?step=1&firstin=1&off=1&keyword4=&code1=&TYPEK=all"
        f"&action=0&queryName=co_id&inpuType=co_id&TYPEK2=&isnew=false"
    )
...
def classify_news_sentiment(title: str, summary: str) -> dict:
    ...
    # 利多關鍵字
    bullish_kws = [
        "獲利", "EPS創高", "EPS成長", "法人買超", "外資買", "獲大單",
        "接單滿載", "擴廠", "新產品", "新客戶", "漲停", "創新高",
        "業績亮眼", "超出預期", "上調目標價", "庫藏股", "股利",
    ]
    # 利空關鍵字
    bearish_kws = [
        "虧損", "跌停", "財報不如預期", "EPS下滑", "外資賣超",
        "法說下修", "獲利衰退", "減資", "重大訴訟", "內線交易",
        "退出市場", "停工", "下調目標價", "裁員",
    ]

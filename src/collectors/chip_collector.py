...
from config import HTTP_HEADERS, HTTP_RETRY, HTTP_TIMEOUT, TWSE_API, TPEX_API, FINMIND_TOKEN
...
def fetch_twse_institutional() -> pd.DataFrame:
    ...
    df_foreign = pd.DataFrame(records).set_index("stock_id") if records else pd.DataFrame()
    ...
    df_trust = pd.DataFrame(records_trust).set_index("stock_id") if records_trust else pd.DataFrame()
    ...
    df_dealer = pd.DataFrame(records_dealer).set_index("stock_id") if records_dealer else pd.DataFrame()

    # 合併
    df = pd.DataFrame(index=list(set(
        list(df_foreign.index) + list(df_trust.index) + list(df_dealer.index)
    )))
...
def fetch_institutional_history_finmind(
    ...
    try:
        ...
        # 外資
        result["foreign_net"] = (
            pivot.get("buy_外陸資買賣超股數(不含外資自營商)", pd.Series(0, index=pivot.index)) -
            pivot.get("sell_外陸資買賣超股數(不含外資自營商)", pd.Series(0, index=pivot.index))
        )

        # 投信
        result["trust_net"] = (
            pivot.get("buy_投信買賣超股數", pd.Series(0, index=pivot.index)) -
...
def fetch_margin_trading(trade_date: Optional[date] = None) -> pd.DataFrame:
    ...
    df = pd.DataFrame(records)
    for col in ["margin_buy", "margin_sell", "margin_balance", "short_sell", "short_buy", "short_balance"]:

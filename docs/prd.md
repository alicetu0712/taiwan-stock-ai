...
✅ 我的交易：手續費計算 + 名稱搜尋（v6.8.15）：
  - 手續費納入損益計算（不改 DB schema，on-the-fly 計算）：
    - 買入：成本總額 = 買入價 × 股數 × 1.001425（含 0.1425% 手續費）
    - 持倉損益：以含手續費成本為基準，標注「未含賣出 0.4425%」
    - 出場損益：賣出淨收入扣 0.4425%（手續費 0.1425% + 證交稅 0.3%）
  - 新增股票改用名稱搜尋：
...
✅ 成本計算重複加費修正（v6.8.17）：
  - 問題：使用者填入的是券商顯示的「均價（已含手續費）」，但系統又額外加一次
    BUY_FEE(0.1425%)，導致成本總額偏高（如 24,935 → 24,971）
  - 修正：eff_buy 直接使用 trade.buy_price（不再乘 1+BUY_FEE）
  - 出場損益的 eff_buy_c 同步修正
  - 輸入欄 label 更新為「買入均價（元，含手續費，依券商顯示填入）」以避免混淆
